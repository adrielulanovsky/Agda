...
{- 
 ...
 revNat-aux x (y ∷ xs) = proof
                         ...
                         ∎
-}
 revNat-aux2 : {X Y : Set}{f : X -> Y}(xs ys : List X) ->
             mapList f (foldl (λ xs x -> x ∷ xs ) xs ys) ≅ foldl (λ xs x -> x ∷ xs ) (mapList f xs) (mapList f ys)
 revNat-aux2 xs [] = refl
 revNat-aux2 {f = f} xs (y ∷ ys) = proof
                mapList f (foldl (λ xs₁ x → x ∷ xs₁) xs (y ∷ ys))
                ≅⟨ revNat-aux2 (y ∷ xs) ys ⟩
                foldl (λ xs₁ x → x ∷ xs₁) (mapList f (y ∷ xs)) (mapList f ys)
                ∎
 
 revNat-proof : {X Y : Set} {f : X -> Y}(a : List X) →
      mapList f (reverse a) ≅ reverse (mapList f a)
 revNat-proof [] = refl
 revNat-proof {f = f} (x ∷ xs) = revNat-aux2 (x ∷ []) xs
{-                      proof
